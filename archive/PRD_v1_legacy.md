# Marathon Trainer — Product Requirements Document (ARCHIVED v1)

**Status**: Archive  
**Last Updated**: 2026-06-17  
**Owner**: Ishi Tripathi  
**Note**: This version has been superseded by PRD.md v2. Kept for historical reference.
...
- Owns: Apple Watch, Oura Ring, Strava account
...
- **Workout hub**: Dashboard includes both running prep and strength training so the app becomes the one-stop shop for training decisions.

### 4.1A Initial Base-Building Protocol
- **Starting point**: 2-minute jog / 2-minute walk intervals
- **Near-term progression**:
  - 2:00 jog / 2:00 walk x 10
  - 2:30 jog / 2:00 walk x 9
  - 3:00 jog / 2:00 walk x 8
  - 3:00 jog / 3:00 walk x 7
- **Progression rule**: Advance only if rib status, perceived effort, recovery, and heart-rate guardrails are acceptable.
- **Regression rule**: Repeat or step down when rib pain, poor recovery, very high effort, or heart-rate spikes appear.
...
**Primary user flow**:
1. User opens app.
2. User voice-notes how the last workout went.
3. App extracts key signals:
   - Rib status
   - Recovery / sleep quality
   - Perceived effort
   - Optional max heart rate
4. App drafts the next recommended run, lift, or recovery day.
5. User can accept, repeat, override, or manually edit.
...
### 4.7 Strength Training for Running
- **Two-session weighted plan**:
  - Strength A: goblet squat, Romanian deadlift, step-up, calf raise, dead bug
  - Strength B: split squat/reverse lunge, hip thrust/glute bridge, dumbbell row, suitcase carry, side plank
- **Training purpose**:
  - Build lower-body strength for impact tolerance
  - Improve single-leg stability
  - Strengthen calves, glutes, hamstrings, and core
  - Support running without making the next run impossible
- **Scheduling principle**: Alternate running, strength, and recovery so stress is distributed across the week.
...
6. One-stop dashboard for run/walk progressions + weighted strength schedule
...
- **Oura Ring integration (MVP)**: Not in the first local MVP; Phase 1B remains high priority after the core check-in flow works
...
### Training Research (In Progress)
- [x] Add source-backed first base-building model: run/walk progression + weighted strength schedule
- [ ] Define the first milestone after 3:00 jog / 3:00 walk
- [ ] Decide whether progression should prioritize longer total session time, shorter walk breaks, or continuous jog duration
...
- **2026-06-17**: Added source-backed run/walk base phase, weighted strength schedule, and one-stop workout dashboard flow
